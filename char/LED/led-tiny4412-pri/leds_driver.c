...
static int __init tiny4412_led_dev_init(void)
{
	...
	for(i=0;i<LED_NUM;i++)
	{
		ret=gpio_request(;ed_gpios[i],"LED");
		if(ret)
		{
		...
		s3c_gpio_cfgpin(led_gpios[i],S3C_GPIO_OUTPUT);//配置为输出模式
		gpio_set_value(led_gpios[i],1);//初始值为高电平
	}
	ret = misc_register(&tiny4412_led_dev);
	
	printk(DEVICE_NAME"\tiny4412_led_dev");
