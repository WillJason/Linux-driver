...
static void tiny4412_buttons_timer(unsigned long _data)
{
	...
	tmp = gpio_get_value(bdata->gpio);//获取端口引脚值

	/* active low */
	down = !tmp;
	printk(KERN_DEBUG "KEY %d: %08x\n", bdata->number, down);//调试输出信息
...
static irqreturn_t button_interrupt(int irq, void *dev_id)
{
	...
	mod_timer(&bdata->timer, jiffies + msecs_to_jiffies(40));//启动定时器，40ms
...
static int tiny4412_buttons_open(struct inode *inode, struct file *file)
{
	...
	{
		if (!buttons[i].gpio)//如果端口未定义，则不申请中断
			continue;
		
		//timer_setup(timer, callback, flags) 用于为第一次使用timer做好准备，一般和add_timer 配合使用，这里是和mod_timer配合使用	
		setup_timer(&buttons[i].timer, tiny4412_buttons_timer,
				(unsigned long)&buttons[i]);
		
		irq = gpio_to_irq(buttons[i].gpio);//将gpio转为对应的irq
		err = request_irq(irq, button_interrupt, IRQ_TYPE_EDGE_BOTH, 
				buttons[i].name, (void *)&buttons[i]);//申请中断
		if (err)
			break;
	}
	...
	if (err) {
		...
		for (; i >= 0; i--) {
			...
			//del_timer_sync函数要完成的任务除了同del_timer一样从
			//定时器队列中删除一个定时器对象外，还会确保当函数返回
			//时系统中没有任何处理器正在执行定时器对象上的定时器函数
		}
		...
		struct poll_table_struct *wait)
{
	...
	/*查询的过程中也是可以休眠，但是一段时间里面休眠，如果有按键按下
	的话，它立刻返回；如果没有按键按下的话，它会等一段时间返回。*/
