...
附加参考：
在嵌入式系统中，如果系统多数时间处于空闲状态，则可考虑采用轮询；否则可考虑采用中断方式来处理触摸屏信
号。本方案采取中断方式，其中断入口程序流程图如图 5 所示。系统初始化时，首先将 TSC2007 设成 PowerDown
 模式，当触摸屏上有点击时，上将产生一个下降脉冲，以引发中断。此时系统进入触摸屏的中断向量。
当中断处理程序判断数据有效后，即设置一个 TI mer ，每隔大约 20 μ s 进行一次 X 、 Y 值的读写，并将取
得的值存入 buffer 中，然后进行去抖、校正处理，同时判断当前状态 (PRESS 、 MOVE 、 RELEASE 等 ) ，最后
将有效的 X 、 Y 值取平均值后发送到系统的消息队列。
*/
